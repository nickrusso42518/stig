...
def check(parse, rule):
    ...
    return {
        "success": success,
        "iter": {"pass": pass_objs, "fail": fail_objs, "na": []},
    }
...
def _check_hier(parse, rule):
    ...
    if fail_objs:
        ...
        success = "PASS"
    return {
        "iter": {"pass": pass_objs, "fail": fail_objs, "na": na_objs},
        "success": success,
    }
...
def process_args():
    ...
    parser.add_argument(
        "config_file", help="configuration text file to scan", type=str
    )
    parser.add_argument(
        "-v",
        "--verbosity",
        type=int,
        choices=[0, 1, 2],
        help="0 for brief, 1 for details, 2 for CSV rows",
        default=0,
    )
    parser.add_argument(
        "-f", "--failonly", help="print failures only", action="store_true"
    )
